...
?>
	<ol class="breadcrumb">
 	...
 	<div class="grid-form">
 		...
 		<h2 id="forms-example" class="">view order Detail</h2>
		...
		//	session_start();
			include("connect.php");
			$id=$_GET["id"];
			$q=mysqli_query($con,"select * from tbl_history where h_id=$id");
			$total=0;
		...
		<?php
			...
			if(isset($_POST["b1"]))
			{
				$id=$_GET["id"];
				extract($_POST);
			$q=mysqli_query($con,"update tbl_history set status='$status' where h_id=$id");
